# Privacy Policy for Cadence

**Effective Date: August 10, 2025**

## 1. Introduction

At Cadence by Tiny Ventures AS ("we," "our," or "us"), we are committed to protecting your privacy and ensuring the security of your personal data. This Privacy Policy explains how we collect, use, store, and protect your information when you use our career development and performance coaching application.

This Privacy Policy is designed to comply with the General Data Protection Regulation (GDPR) and other applicable privacy laws. By using Cadence, you consent to the data practices described in this policy.

**Data Controller**: Cadence by Tiny Ventures AS, Røahagan 1P, 0754 Oslo, Norway  
**Data Protection Officer**: kasper.herloev@gmail.com

## 2. Information We Collect

### 2.1 Personal Information You Provide

- **Account Information**: Email address, password, and account credentials
- **Profile Information**: Name, job title, company, career details, and professional background
- **Career Development Data**: Career goals, skills assessments, progress tracking, and personal reflections
- **Communication Data**: Messages, feedback, and support requests you send to us

### 2.2 Information We Collect Automatically

- **Usage Data**: How you interact with our application, features used, and time spent
- **Technical Data**: IP address, browser type, device information, and operating system
- **Performance Data**: Application performance, error logs, and system diagnostics
- **AI Interaction Data**: Coaching conversations, prompts, and AI-generated responses

### 2.3 Special Category Data

We process sensitive personal data that may include:
- Personal career aspirations and life goals
- Professional assessments and performance evaluations
- Intimate thoughts and reflections about career development
- Personal challenges and vulnerabilities shared during coaching
- Skill assessments and competency evaluations

We process this sensitive data only with your explicit consent and implement additional safeguards to protect it.

## 3. How We Use Your Information

### 3.1 Primary Purposes

- **Service Provision**: To provide and maintain our career development platform
- **AI Coaching**: To generate personalized coaching insights and career advice
- **Progress Tracking**: To help you monitor your career development journey
- **Account Management**: To manage your account, subscriptions, and billing

### 3.2 Secondary Purposes

- **Service Improvement**: To analyze usage patterns and improve our services
- **Security**: To protect against fraud, abuse, and security threats
- **Support**: To provide customer support and respond to inquiries
- **Legal Compliance**: To comply with legal obligations and regulatory requirements

### 3.3 Legal Basis for Processing

We process your personal data based on the following legal grounds:

| Purpose | Legal Basis | Data Types |
|---------|-------------|------------|
| Account creation and management | Contract performance | Account information, profile data |
| AI coaching and insights | Explicit consent | Career data, reflections, AI interactions |
| Payment processing | Contract performance | Billing information (processed by Stripe) |
| Service improvement | Legitimate interest | Usage data, performance metrics |
| Security and fraud prevention | Legitimate interest | Technical data, usage patterns |
| Legal compliance | Legal obligation | Account data, transaction records |

## 4. Data Sharing and Third Parties

### 4.1 Third-Party Service Providers

We use trusted third-party services to operate our platform:

| Service Provider | Purpose | Data Shared | Privacy Policy |
|------------------|---------|-------------|----------------|
| **Supabase** | Database hosting and authentication | All personal data and application data | [View Policy](https://supabase.com/privacy) |
| **Stripe** | Payment processing | Payment information (we never see this data) | [View Policy](https://stripe.com/privacy) |
| **Anthropic** | AI coaching and insights | Career data and coaching conversations | [View Policy](https://www.anthropic.com/privacy) |

### 4.2 Data Processing Agreements

All third-party service providers are bound by data processing agreements that ensure:
- Compliance with GDPR requirements
- Appropriate security measures
- Confidentiality obligations
- Data deletion upon contract termination

### 4.3 International Data Transfers

Your data may be transferred to and processed in countries outside the European Economic Area (EEA). We ensure adequate protection through:
- **Standard Contractual Clauses (SCCs)**: Approved by the European Commission
- **Adequacy Decisions**: Where applicable
- **Appropriate Safeguards**: Additional security measures as required

### 4.4 Legal Disclosures

We may disclose your information in the following circumstances:
- To comply with legal obligations or court orders
- To protect our rights, property, or safety
- To prevent fraud or security threats
- With your explicit consent

## 5. Data Security

### 5.1 Security Measures

We implement comprehensive security measures to protect your data:
- **Encryption**: All data is encrypted in transit (HTTPS) and at rest
- **Access Controls**: Role-based access with least privilege principles
- **Authentication**: Secure authentication and session management
- **Monitoring**: Continuous security monitoring and threat detection
- **Backup Security**: Encrypted backups with secure storage

### 5.2 Data Breach Response

In the event of a data breach, we will:
- Notify relevant authorities within 72 hours of discovery
- Notify affected users without undue delay
- Take immediate steps to contain and remediate the breach
- Conduct a thorough investigation and implement preventive measures

## 6. Your Rights (GDPR)

### 6.1 Access and Portability

- **Right of Access**: Request a copy of all personal data we hold about you
- **Right to Portability**: Receive your data in a structured, machine-readable format
- **Data Export**: Export your data in JSON or Markdown formats anytime

### 6.2 Data Control

- **Right to Rectification**: Request correction of inaccurate or incomplete data
- **Right to Erasure**: Request deletion of your account and all associated data
- **Right to Restriction**: Request limitation of data processing
- **Right to Object**: Object to certain types of data processing

### 6.3 Consent and Withdrawal

- **Right to Withdraw Consent**: Withdraw consent for data processing at any time
- **Right to Opt-Out**: Opt out of certain data processing activities
- **Marketing Preferences**: Control marketing communications

### 6.4 Automated Decision Making

- **Right to Human Review**: Request human review of AI-generated insights
- **Right to Explanation**: Understand how AI decisions are made
- **Right to Challenge**: Challenge automated decisions that affect you

### 6.5 How to Exercise Your Rights

To exercise any of these rights, please contact us at [kasper.herloev@gmail.com](mailto:kasper.herloev@gmail.com). We will respond to your request within 30 days.

## 7. Data Retention

### 7.1 Retention Periods

| Data Type | Retention Period | Reason |
|-----------|-----------------|---------|
| Account data | Duration of account + 30 days | Service provision |
| Career development data | Duration of account + 30 days | Service provision |
| AI conversation logs | Duration of account + 30 days | Service improvement |
| Usage analytics | 2 years | Service improvement |
| Billing records | 7 years | Legal compliance |
| Backup data | 30 days after deletion | Disaster recovery |

### 7.2 Data Deletion

- Account deletion requests are processed within 30 days
- All associated data is permanently removed
- Backup data is deleted within 30 days
- Some data may be retained longer for legal compliance

## 8. Cookies and Tracking

### 8.1 Essential Cookies

- **Authentication**: Session management and login status
- **Security**: CSRF protection and security tokens
- **Functionality**: User preferences and settings

### 8.2 Analytics Cookies

- **Usage Analytics**: To understand how users interact with our service
- **Performance Monitoring**: To identify and fix technical issues
- **Service Improvement**: To enhance user experience

### 8.3 Cookie Management

You can control cookies through your browser settings. However, disabling essential cookies may affect service functionality.

## 9. Children's Privacy

Cadence is not intended for children under 18 years of age. We do not knowingly collect personal information from children under 18. If you believe we have collected such information, please contact us immediately.

## 10. Changes to This Privacy Policy

We may update this Privacy Policy from time to time. We will notify you of any material changes by:
- Email notification to your registered email address
- Prominent notice on our website
- In-app notification

Continued use of our service after changes constitutes acceptance of the updated policy.

## Contact Information

**Data Controller**: Cadence by Tiny Ventures AS  
**Address**: Røahagan 1P, 0754 Oslo, Norway  
**Email**: [kasper.herloev@gmail.com](mailto:kasper.herloev@gmail.com)  
**Website**: [https://hellocadence.com](https://hellocadence.com)  
**Data Protection Officer**: [kasper.herloev@gmail.com](mailto:kasper.herloev@gmail.com)  
**Terms of Service**: [View our Terms of Service](../docs/terms-of-service.md)

### Supervisory Authority

If you have concerns about our data processing, you have the right to lodge a complaint with your local data protection authority:

- **Norway**: [Datatilsynet](https://www.datatilsynet.no/)
- **EU Users**: Your local data protection authority

---

**Last Updated**: August 10, 2025

This Privacy Policy is effective as of the date stated above. 